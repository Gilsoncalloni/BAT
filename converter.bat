...
set pasta=C:\temp
REM md %pasta%\old
REM copy %pasta%\*.odt %pasta%\old\ /y  
dir /s %pasta%\*.odt
for /f  "tokens=1* delims=|" %%b in ('"dir /s /b %pasta%\*.odt "') do "C:\Program Files\LibreOffice\program\swriter.exe" --headless --convert-to doc "%%b" --outdir "%%~dpb"
dir /s %pasta%\*.odt
dir /s %pasta%\*.doc
pause
